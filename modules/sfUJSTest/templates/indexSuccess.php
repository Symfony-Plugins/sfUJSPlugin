...
testSuite = function(){
  ...
  test("Placeholder, replacer and writer", function() {
    ...
    is($('#test_case8').html().toLowerCase(), "<b>success</b>", "UJS_replace() replaces existing elements matching selector with given HTML" );
    is_deeply($('#test_case8_inside'), [], "UJS_replace() removes existing elements matching selector " );
    is($('#test_case9').html().toLowerCase(), '<span class="foo" name="bar">success</span>', "UJS_replace() espaces single and double quotes in second argument properly" );
    is($('#test_case10').html(), "success", "UJS_write() writes into the document" );
    is($('#test_case11').html().toLowerCase(), "<b>success</b>", "UJS_write() writes HTML into the document" );
    is($('#test_case12').html().toLowerCase(), "<b>success</b>", "UJS_write_block() writes HTML into the document" );
  });
